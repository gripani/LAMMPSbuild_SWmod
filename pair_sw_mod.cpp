...
/* ----------------------------------------------------------------------
   ...
   Modified version by Giorgio Ripani
   Angular term g(Cos t) = g0,n + chi,n * (Cos t - Cos teq,n)^2 , n = [1,N]
------------------------------------------------------------------------- */
...
PairSWmod::PairSWmod(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairSWmod::coeff(int narg, char **arg)
{
  int i,j,n;
...
double PairSWmod::init_one(int i, int j)
{
...
void PairSWmod::read_file(char *file)
{
  ...
  while (1) {
    ...
    params[nparams].switc = atoi(words[14]);
...
void PairSWmod::twobody(Param *param, double rsq, double &fforce,
...
/* Check if lambda=0 then do not compute */
  double le;
  le = paramijk->lambda_epsilon;
...
/* ------------------------------------- */
  ...
  if(paramijk->switc == 1) {
        ...
        double g0[]    = {                 .0,           1.0/12.0,                 .0 };
        double chi[]   = {                1.0,               -1.0,                1.0 };
        double costh[] = {     -sqrt(2.0/3.0),     -1.0/sqrt(6.0),                 .0 };
        double xmin[]  = {              -1.00, -sqrt(3.0/2.0)/2.0, -1.0/(2*sqrt(6.0)) };
        double xmax[]  = { -sqrt(3.0/2.0)/2.0, -1.0/(2*sqrt(6.0)),               1.00 };

  	for (int m=0; m < 3; m++) {
		if ( ( cs >= xmin[m] ) && ( cs < xmax[m] ) ) {
			delcs = cs - costh[m];
	  		delcssq = delcs*delcs;

			facexp = expgsrainv1 * expgsrainv2;
	
	  		facrad = paramijk->lambda_epsilon * facexp * (g0[m] + chi[m] * delcssq);
	  		frad1 = facrad*gsrainvsq1;
	  		frad2 = facrad*gsrainvsq2;
	  		facang = paramijk->lambda_epsilon2 * facexp *  chi[m] * delcs;
  ...
  if(paramijk->switc == 2) {
        ...
        double g0[]    = { .00000, .03125, .00000, .03125, .00000, .03125, .00000 };
        double chi[]   = { 1.0000,-1.0000, 1.0000,-1.0000, 1.0000,-1.0000, 1.0000 };
        double costh[] = {-1.0000,-0.7500,-0.5000,-0.2500, 0.0000, 0.2500, 0.5000 };
        double xmin[]  = {-1.0000,-0.8750,-0.6250,-0.3750,-0.1250, 0.1250, 0.3750 };
        double xmax[]  = {-0.8750,-0.6250,-0.3750,-0.1250, 0.1250, 0.3750, 1.0000 };
